...
func (c *User) Delete(ctx context.Context, tx *sql.Tx, user *user.RootUserModel, deleteFromUserID vo.UserID) error {
	...
	dtoObject := c.toDTO(user, deleteFromUserID, rdb.IN_ACTIVE)
...
func (c *User) FindAll(ctx context.Context) (user.RootUserModelSlice, error) {
	...
	usersDTOs, err := dto.TBLUsers(
		dto.TBLUserWhere.DeleteFlag.EQ(rdb.ACTIVE),
...
func (d *User) toModel(record *dto.TBLUser) (*user.RootUserModel, error) {
	...
	errs = errors.Join(errs, utility.SetVOConstructor(&id, vo.NewUserID, record.ID))
	errs = errors.Join(errs, utility.SetVOConstructor(&roleKey, vo.NewRoleKey, record.RoleKey))
	errs = errors.Join(errs, utility.SetVOConstructor(&userName, vo.NewUserName, record.UserName))
	errs = errors.Join(errs, utility.SetVOConstructor(&password, vo.ReconstructHashedPassword, record.Password))
	errs = errors.Join(errs, utility.SetVOConstructor(&displayName, vo.NewUserDisplayName, record.Name))
