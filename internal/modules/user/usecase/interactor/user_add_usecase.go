...
type IUserAddPort interface {
	Execute(ctx context.Context, inputAddUser UserAddInput, inputRole string, inputAddFromUserID int) error
}
...
func (r UserAddInteractor) Execute(ctx context.Context, inputAddUser UserAddInput, inputRole string, inputAddFromUserID int) error {
	...
	addFromUserID, err := vo.NewUserID(inputAddFromUserID)
	if err != nil {
		return log.WrapErrorWithStackTraceBadRequest(err)
	}
...
func (a UserAddInteractor) createUserModel(inputAddUser UserAddInput) (*user.RootUserModel, error) {
	...
	errs = errors.Join(errs, utility.SetVOConstructor(&roleKey, vo.NewRoleKey, inputAddUser.RoleKey))
	errs = errors.Join(errs, utility.SetVOConstructor(&userName, vo.NewUserName, inputAddUser.UserName))

	password, err := vo.NewPasswordForCreation(inputAddUser.Password)
	errs = errors.Join(errs, err)

	errs = errors.Join(errs, utility.SetVOConstructor(&displayName, vo.NewUserDisplayName, inputAddUser.DisplayName))
