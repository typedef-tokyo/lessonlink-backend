...
type (
	...
	UserLoginController struct {
		inputPort interactor.IUserLoginInputPort
...
func (h *UserLoginController) Execute(c echo.Context) error {
	...
	// ユースケースを実行
	result, err := h.inputPort.Execute(c.Request().Context(), interactor.UserLoginInput{UserName: userName, UserRawPassword: password})
