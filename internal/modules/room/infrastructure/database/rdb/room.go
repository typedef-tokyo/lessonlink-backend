...
func NewRoomRepository(c rdb.IMySQL) repository.RoomRepository {
...
func (f *Room) toModel(record *dto.DataRoom) (*room.RootRoomModel, error) {
	...
	errs = errors.Join(errs, utility.SetVOConstructor(&index, vo.NewRoomIndex, record.RoomIndex))
	errs = errors.Join(errs, utility.SetVOConstructor(&name, vo.NewRoomName, record.Name))
