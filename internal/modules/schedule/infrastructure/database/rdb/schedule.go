...
func NewScheduleRepository(c rdb.IMySQL) repository.ScheduleRepository {
...
func (f *Schedule) toModel(record *dto.TBLSchedule) (*schedule.RootScheduleModel, error) {
	...
	var errs error
	errs = errors.Join(errs, utility.SetVOConstructor(&id, vo.NewScheduleID, record.ID))
	errs = errors.Join(errs, utility.SetVOConstructor(&campus, vo.NewCampus, record.Campus))
	errs = errors.Join(errs, utility.SetVOConstructor(&title, vo.NewScheduleTitle, record.Title))
	errs = errors.Join(errs, utility.SetVOConstructor(&historyIndex, vo.NewHistoryIndex, record.HistoryIndex))
	errs = errors.Join(errs, utility.SetVOConstructor(&createUser, vo.NewUserID, record.CreateUser))
	errs = errors.Join(errs, utility.SetVOConstructor(&lastUpdateUser, vo.NewUserID, record.LastUpdateUser))
	...
	if record.R != nil {
		...
		for _, recordItem := range record.R.ScheduleTBLScheduleItems {
			...
			var errs error
			errs = errors.Join(errs, utility.SetVOConstructor(&lessonID, vo.NewLessonID, recordItem.LessonID))
			errs = errors.Join(errs, utility.SetVOConstructor(&identifier, vo.NewIdentifier, recordItem.Identifier))
			errs = errors.Join(errs, utility.SetVOConstructor(&duration, vo.NewLessonDuration, recordItem.Duration))
		...
		for _, recordItem := range record.R.ScheduleTBLScheduleRoomItems {
			...
			var errs error
			errs = errors.Join(errs, utility.SetVOConstructor(&itemTag, vo.NewRoomItemTag, recordItem.ItemTag))
			errs = errors.Join(errs, utility.SetVOConstructor(&lessonID, vo.NewLessonID, recordItem.LessonID))
			errs = errors.Join(errs, utility.SetVOConstructor(&identifier, vo.NewIdentifier, recordItem.Identifier))
			errs = errors.Join(errs, utility.SetVOConstructor(&duration, vo.NewLessonDuration, recordItem.Duration))
			...
			errs = errors.Join(errs, utility.SetVOConstructor(&roomIndex, vo.NewRoomIndex, recordItem.RoomIndex))
