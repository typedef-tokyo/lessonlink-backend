...
type (
	...
	ScheduleItemJoinController struct {
		inputPort interactor.IScheduleItemJoinInputPort
...
func (h *ScheduleItemJoinController) Execute(c echo.Context) error {
	...
	result, err := h.inputPort.Execute(c.Request().Context(), userID, scheduleID, requestData.HistoryIndex, requestData.JoinFromIdentifier, requestData.JoinToIdentifier)
