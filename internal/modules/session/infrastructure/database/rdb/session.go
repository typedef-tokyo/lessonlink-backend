...
func NewSessionRepository(c rdb.IMySQL) repository.SessionRepository {
	return &Session{c: c.GetConn()}
}

func (f *Session) Save(ctx context.Context, tx *sql.Tx, session *model.SessionModel) error {
	...
	if tx != nil {
		...
		err = sessionDTO.Insert(ctx, f.c, boil.Infer())
	}
...
func (f *Session) Update(ctx context.Context, tx *sql.Tx, session *model.SessionModel) error {
...
func (f *Session) Find(ctx context.Context, sessionID string) (*model.SessionModel, error) {
	...
	var entity *model.SessionModel
	for _, dto := range sessionDTO {

		entity, err = f.toModel(dto)
		if err != nil {
...
func (f *Session) toModel(session *dto.SysSession) (*model.SessionModel, error) {
	...
	userID, err := vo.NewUserID(v.UserID)
	if err != nil {
		return nil, log.WrapErrorWithStackTraceInternalServerError(err)
	}

	roleKey, err := vo.NewRoleKey(v.RoleKey)
	if err != nil {
		return nil, log.WrapErrorWithStackTraceInternalServerError(err)
	}
...
func (f *Session) toDTO(session *model.SessionModel) (*dto.SysSession, error) {
